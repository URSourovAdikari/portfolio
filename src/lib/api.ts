...
const MUTATION_PROXY_PATHS = new Set(["/api/contact", "/api/newsletter/subscribe"]);
...
async function request<T>(path: string, init?: RequestInit, key?: string): Promise<T> { const controller = new AbortController(); const timeout = globalThis.setTimeout(() => controller.abort(), 12000); const requestUrl = MUTATION_PROXY_PATHS.has(path) ? path : `${API_URL}${path}`; try { const response = await fetch(requestUrl, { ...init, signal: init?.signal ?? controller.signal, headers: { "Content-Type": "application/json", ...init?.headers } }); const payload: unknown = await response.json().catch(() => undefined); if (!response.ok) throw new Error((payload as { error?: { message?: string } } | undefined)?.error?.message ?? (payload as { message?: string } | undefined)?.message ?? "Portfolio API request failed"); return unwrap(payload as T | ApiEnvelope<T>, key); } catch (error) { if (error instanceof DOMException && error.name === "AbortError") throw new Error("Portfolio API request timed out", { cause: error }); throw error; } finally { globalThis.clearTimeout(timeout); } }

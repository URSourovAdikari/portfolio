...
async function request<T>(path: string, init?: RequestInit, key?: string): Promise<T> { const controller = new AbortController(); const timeout = globalThis.setTimeout(() => controller.abort(), 12000); try { const response = await fetch(`${API_URL}${path}`, { ...init, signal: init?.signal ?? controller.signal, headers: { "Content-Type": "application/json", ...init?.headers } }); const payload: unknown = await response.json().catch(() => undefined); if (!response.ok) throw new Error((payload as { error?: { message?: string } } | undefined)?.error?.message ?? "Portfolio API request failed"); return unwrap(payload as T | ApiEnvelope<T>, key); } catch (error) { if (error instanceof DOMException && error.name === "AbortError") throw new Error("Portfolio API request timed out"); throw error; } finally { globalThis.clearTimeout(timeout); } }

...
export const AboutSection = ({ portfolio, showImage = true }: { portfolio: Portfolio; showImage?: boolean }) => {
  ...
  return (
    <section id="about" className="about-section max-w-7xl mx-auto w-full px-6 py-24 md:py-28">
      <motion.div className="about-intro flex flex-col md:flex-row gap-10 lg:gap-16 items-center" initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: "easeOut" }} viewport={{ once: true, amount: 0.2 }}>
        {showImage && (
        ...
        <div className="about-copy flex-1 space-y-8">
          <div>
        ...
        <div className="about-stats flex-1 grid grid-cols-2 gap-4 w-full">
          {stats.map((stat, i) => (
